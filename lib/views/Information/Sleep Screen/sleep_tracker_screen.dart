...
class _SleepTrackerScreenState extends State<SleepTrackerScreen> {
  ...
  void _setupSleepListeners() {
    _sleepUpdateSubscription =
        _service.on('sleep_update').listen((event) async {
    ...
    _sleepSavedSubscription =
        _service.on('sleep_saved').listen((event) async {
          if (event != null && mounted) {
    ...
    _goalReachedSubscription =
        _service.on('sleep_goal_reached').listen((event) {
          if (event != null && mounted) {
  ...
  void _handleSecretSleepPush() {
    ...
    if (bedTime == null || wakeTime == null) {
      Get.snackbar(
          'Sleep times missing', 'Set both bedtime and wake time first.',
          snackPosition: SnackPosition.BOTTOM);
      _secretTapCount = 0;
      return;
    }
                      ...
                      children: [
                        ...
                        Obx(() =>
                            Row(
                              ...
                              children: [
                                Text(_fmt(sleepController.bedtime.value),
                      ...
                      children: [
                        ...
                        Obx(() =>
                            Row(
                              crossAxisAlignment: CrossAxisAlignment.center,
                              children: [
                                Text(_fmt(sleepController.waketime.value),
                                    style: const TextStyle(
                                        fontSize: 16,
                                        fontWeight: FontWeight.w400)),
                                IconButton(
                    ...
                    children: [
                      ...
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            const Text('View Sleep Report',
                                style: TextStyle(
                                    fontSize: 16,
                                    fontWeight: FontWeight.w600)),
                            const SizedBox(height: 4),
                            Text('Detailed analysis of your sleep stages',
                                style: TextStyle(
    ...
    return CircularPercentIndicator(
      ...
      center: Column(
        ...
        children: [
          ...
          Text(_formatDuration(_currentSleepDuration),
              style: const TextStyle(
                  fontSize: 28, fontWeight: FontWeight.bold)),
          Text('of ${_formatDuration(_sleepGoal)}',
        ...
        center: Column(
          ...
          children: [
            d.inMinutes > 0
                ...
                style: const TextStyle(
                    fontSize: 28, fontWeight: FontWeight.bold))
                : const Text('No sleep yet',
                style: TextStyle(
                    fontSize: 20,
                    fontWeight: FontWeight.w600,
                    color: Colors.grey)),
            const SizedBox(height: 8),
            Text(sleepController.getSleepStatus(d),
                style: TextStyle(fontSize: 14, color: Colors.grey[600])),
          ],
        ),
      );
