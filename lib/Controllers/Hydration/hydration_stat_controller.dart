...
// Save water intake value locally
  ...
  prefs.setString('lastUpdatedDate', DateFormat('yyyy-MM-dd').format(DateTime.now()));
...
Future<void> loadWaterIntake() async {
  ...
  void updateMasterCheckbox() {
    masterCheck.value =
        waterHistoryList.every((item) => item.isChecked.value == true);
  }
  ...
  Future<void> updateWaterGoal(int value) async {
    waterGoal.value = value; // ✅ Update water goal
    await saveWatergoal(WaterGoalVM(
  ...
  Future<void> saveWatergoal(WaterGoalVM water) async {
  try {
    ...
    if (response is http.Response) {
      Get.snackbar('Error', 'Failed to save Water goal: ${response.statusCode}');
      return;
    }

    // Get.snackbar('Success', 'Water goal saved successfully!');

    print("✅ Water goal saved successfully");

  } catch (e) {
    Get.snackbar('Error', 'Failed saving Water goal');
  }
}

  Future<void> saveWaterRecord(int count) async {
  try {
    ...
    if (response is http.Response && response.statusCode >= 400) {
      Get.snackbar('Error','❌ Failed to save Water record: ${response.statusCode}');
    } else {
      // Get.snackbar('Success','✅ Water record saved successfully');
    }
  } catch (e) {
    Get.snackbar('Error','❌ Exception while saving Water record');
  }
}
...
Future<void> fetchWaterRecords() async {
  try {
    ...
    if (response is http.Response && response.statusCode >= 400) {
      Get.snackbar('Error','❌ Failed to fetch Water records: ${response.statusCode}');
      return;
    }
